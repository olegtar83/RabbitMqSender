...
5. Заходим в админку реббита `http://localhost:15672/` c паролью и юзером `guest guest` и наблюдаем за активностью.
6. Тем временем можно посмотреть происходящие в логах `http://localhost:8020/` и увидеть ошибку обращения к несуществующему адресу `https://somesite/api/v1/invoice` а так же посмотреть на передаваемый xml и запись json в бд.
7. Затем зайти в psql
   ```
